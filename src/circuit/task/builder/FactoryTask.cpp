...
void CBFactoryTask::Finish()
{
	...
	if (std::string("factoryplane") == buildDef->GetUnitDef()->GetName()) {
		circuit->GetScheduler()->RunTaskAfter(std::make_shared<CGameTask>("CBFactoryTask_repairpad", [circuit, pos]() {
		...
		circuit->GetScheduler()->RunTaskAfter(std::make_shared<CGameTask>("CBFactoryTask_nano", [circuit, pos]() {
